...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-r from-slate-950 via-stone-900 to-orange-950/90 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        <div class="absolute -right-10 -bottom-10 w-64 h-64 bg-orange-500/20 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-6 relative z-10">
            ...
            <div class="relative group">
                @if($user->image && file_exists(public_path('uploads/profile/' . $user->image)))
                    <img src="{{ asset('uploads/profile/' . $user->image) }}" 
                         alt="{{ $user->name }}" 
                         class="w-28 h-28 rounded-2xl object-cover ring-4 ring-orange-500/30 shadow-2xl">
                @else
                    <div class="w-28 h-28 rounded-2xl bg-orange-500/20 ring-4 ring-orange-500/30 flex items-center justify-center font-extrabold text-3xl text-orange-300 shadow-2xl">
            ...
            <div class="text-center sm:text-left flex-1">
                <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-orange-500/20 text-orange-300 border border-orange-400/20 mb-2">
                    Kandidat Pelamar
                </div>
                <!-- a. Nama Kandidat -->
                <h2 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">{{ $user->name }}</h2>
                <!-- b. Position Kandidat -->
                <p class="text-orange-200 font-medium text-base mt-1">{{ $user->position ?? 'IT Fullstack Developer' }}</p>
                ...
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-4 mt-4 text-xs text-slate-300">
                    ...
                    <span class="flex items-center">
                        <svg class="w-4 h-4 mr-1.5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
